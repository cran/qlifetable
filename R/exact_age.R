...
#'
#' @description  Computes the time(s) elapsed (in years) between the date(s) of birth and the date(s) of event(s).
#'
...
#'                   of the population the exact age in the moment of the event (e.g., death).
...
#' each year.
#'
#' @seealso \code{\link{coord_age}}, \code{\link{coord_time}}
#'
#' @export
#'
...
exact_age <- function(date.birth, date.event, random.b = TRUE, random.e = TRUE, constant.age.year = FALSE){
  ...
  Coord.y <-  date.event - date.birth

  neg <- sum(Coord.y < 0L)
  if (neg > 0L)
    stop(paste0("There are ", neg, " observations for which the dates of events are previous to the dates of birth."))

  if (constant.age.year){
    length.year <- 365.25
  } else {
    length.year <- length_year(date.birth, date.event)
  }

  Coord.y <- as.double(Coord.y / time_year(Coord.y, "y", year = length.year))

#  if ((!random.b & random.e) | (random.b & !random.e)) {
#    same <- which(date.birth > date.event)
#    birth <- date.birth[same]
#    event <- date.event[same]
#    Coord.y[same] <- same_day(birth, event, random.b, random.e)
#  }

#  if (random.b & random.e){
#      Coord.y[Coord.y < 0L] <- -Coord.y[Coord.y < 0L]
#  }
  return(Coord.y)
}
